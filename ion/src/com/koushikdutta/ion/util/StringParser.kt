...
class StringSerializer(val charset: Charset = Charsets.UTF_8, val contentType: String = "text/plain") : AsyncSerializer<String> {
    override fun write(value: String): Promise<AsyncHttpMessageContent> = ByteBufferListSerializer(contentType).write(ByteBufferList(ByteBuffer.wrap(value.toByteArray(charset))))
}
